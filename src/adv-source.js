...
const AdvSourceMixin = (superclass) =>
      ...
      return {
        add: (changes, advKeys, disKeys) => {
          changes.forEach((change) => {
            if (advKeys.includes(change.key))
              advantageLabels.push(change.label);
            if (disKeys.includes(change.key))
              disadvantageLabels.push(change.label);
          });
        },
        ...
        update: (options) => {
          ...
          if (advantageLabels.length)
            setProperty(
              options,
              "dialogOptions.adv-reminder.advantageLabels",
              advantageLabels
            );
...
export class AttackSource extends AdvSourceMixin(AttackReminder) {}
